...
export default async function RobotsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
      <h1 className="text-3xl font-bold text-white mb-2">Robot & Automation Job Loss Tracker</h1>
      <p className="text-gray-400 mb-8">Tracking job displacement from industrial automation, robotics, humanoid robots, and autonomous systems.</p>

      <div className="bg-gradient-to-b from-purple-950/30 to-transparent rounded-xl p-8 text-center mb-12">
        {total > 0 ? (
          <>
            <p className="text-purple-400/70 text-sm uppercase tracking-widest mb-2">Estimated Robot/Automation Jobs Lost</p>
            <div className="text-5xl font-black text-purple-400 tabular-nums">{formatNumberFull(total)}</div>
          ...
          <>
            ...
            <p className="text-gray-600 text-sm mt-2">Robot and automation displacement events will appear here as they are discovered.</p>
          </>
        )}
      </div>
        ...
        <div className="space-y-3">
          {events.map((event) => (
            <div key={event.id} className="bg-slate-900/50 border border-slate-800 rounded-lg p-4 flex items-center justify-between">
              ...
              <div className="text-purple-400 font-bold tabular-nums">{event.weightedAiJobs.toLocaleString()}</div>
            </div>
          ))}
        </div>
      ) : (
